...
import "./MpcCore.sol";
import {GCDecryptionVerifier} from "./MpcInterface.sol";
import {BubbleAddresses} from "./BubbleAddresses.sol";
...
abstract contract DecryptionCaller {
    ...
    function verifySignatures(uint256[] memory handles, bytes[] memory outputs, bytes[] memory signatures) internal returns (bool) {
        return GCDecryptionVerifier(BubbleAddresses.gcDecryptionVerifier()).verifyDecryptionSignatures(handles, outputs, signatures);
    }
